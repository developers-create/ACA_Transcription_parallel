...
def download_audio_files(bridge_ids, token):
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Total call recordings to download: {len(bridge_ids)}")
    logger.info(f"{'='*60}\n")
    ...
    for idx, bridge_id in enumerate(bridge_ids):
        ...
        try:
            logger.info(f"⬇ [{idx+1}/{len(bridge_ids)}] Downloading: {callid}.wav")
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Download complete: {len(downloaded_files)}/{len(bridge_ids)} files")
    logger.info(f"{'='*60}\n")
...
def process_audio_files():
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Processing {len(files)} audio files sequentially")
    logger.info(f"{'='*60}\n")
    
    for idx, file in enumerate(files):
        try:
            logger.info(f"🎙️ [{idx+1}/{len(files)}] Processing: {file}")
            
            with open(f"./input_file/{file}", "rb") as f_audio:
                audio_data = base64.b64encode(f_audio.read()).decode("utf-8")
            ...
            call_id = str(file).split('.')[0]
            response_dict['call_id'] = call_id
            data.append(response_dict)
            
            logger.info(f"  ✓ Processed: {call_id}")
            
        except Exception as e:
            logger.error(f"  ✗ Error processing {file}: {str(e)}")
    
    logger.info(f"\n{'='*60}")
    logger.info(f"Processing complete: {len(data)}/{len(files)} successful")
    logger.info(f"{'='*60}\n")
...
def save_output_json(data):
    ...
    with open(f"./processed_reports/output.json", "w", encoding='utf-8') as f:
...
def evaluate_bank_info(conversation_data, engine):
    ...
    if _DOCTOR_DF_CACHE is None:
        try:
            logger.info("Fetching Doctor Schedule from DB")
            ...
            logger.info("Doctor Schedule Cached.")
        except Exception as e:
...
def calculate_call_score(conversation_text):
    ...
    closing_phrases = [
        ...
        "sewa ka mauka", "dhanyavad", "dhanyawad", "thank you", "thanks",
        "Aapka samay dene ke liye dhanyawad", "dhanyawaad", "dhanyavaad"
    ]
...
def setup_database():
    ...
    engine = create_engine(DATABASE_URL_CONN)
...
def insert_call_details(db, call_with_call_id, Call_Details):
    ...
    for item in call_with_call_id:
        ...
        if isinstance(item, dict):
            ...
            db.add(record)
            new_count += 1
            logger.info(f"added id:{bridge_id}")
    
    db.commit()
    logger.info(f"Inserted {new_count} new records, skipped {skipped_count} duplicates")

def insert_transcription_records(db, data, TranscriptionRecord):
    logger.info("Adding in Transcription Record")
    for item in data:
        ...
        db.add(record)
        logger.info(f"{item['call_id']} Saved to Database.")
...
async def transcript_calls(request: ManualTranscriptRequest):
    try:
        ...
        logger.info(f"✓ Total calls requested: {len(call_ids)}")
        logger.info(f"✓ Audio files downloaded: {len(downloaded_files)}")
        logger.info(f"✓ Calls processed: {len(data)}")
        logger.info(f"✓ Records saved to database: {len(data)}")
        logger.info(f"{'='*60}\n")
        
        return TranscriptResponse(
            success=True,
            message=f"Successfully processed {len(data)} calls",
            total_requested=len(call_ids),
            downloaded=len(downloaded_files),
            processed=len(data),
            saved_to_db=len(data)
        )
...
def main(process_date=None):
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Processing calls for date: {process_date}")
    logger.info(f"{'='*60}\n")
    ...
    logger.info(f"Found {len(call_with_call_id)} calls with bridge IDs")
    
    if len(call_with_call_id) == 0:
        logger.info("No calls found for processing. Exiting.")
        return
    
    downloaded_files = download_audio_files(call_with_call_id, token)
    
    if len(downloaded_files) == 0:
        logger.info("No audio files downloaded. Exiting.")
        return
    
    data = process_audio_files()
    
    if len(data) == 0:
        logger.info("No files processed successfully. Exiting.")
        return
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"✓ Processing complete!")
    logger.info(f"✓ Total calls found: {len(call_with_call_id)}")
    logger.info(f"✓ Audio files downloaded: {len(downloaded_files)}")
    logger.info(f"✓ Calls processed: {len(data)}")
    logger.info(f"✓ Records saved to database: {len(data)}")
    logger.info(f"{'='*60}\n")
